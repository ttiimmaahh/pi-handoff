...
```bash
pi install git:github.com/<your-user>/pi-handoff@main
```
...
> existing install, `git -C ~/.pi/agent/git/github.com/<you>/pi-handoff pull`
> and restart pi.

## Commands

- `/handoff-setup` — guided setup (like `/login`): pick the **summarizer model**
  (listed cheapest-first with per-Mtok cost) and the **trigger threshold**.
  Saved to a config file so it sticks across sessions.
...
```json
{ "model": "sap-aicore/anthropic--claude-4.5-haiku", "threshold": 80 }
```
